...
public class AndroidUtil {
    ...
    public static boolean isAutoGenMethod(SootMethod sootMethod){
        String clsSig = sootMethod.getDeclaringClass().getName();
        List<String> androidPrefixPkgNames = Arrays.asList("R$");
        return (androidPrefixPkgNames.stream().map(clsSig::contains).reduce(false, (res, curr) -> res || curr) || clsSig.endsWith(".R") || clsSig.endsWith(".BuildConfig"));
    }
